...
public class ThreadPoolDownloadService extends Service {
	...
	private static final String TAG = ThreadPoolDownloadService.class.getSimpleName();
	...
	public int onStartCommand(final Intent intent,
        ...
        Runnable downloadRunnable = new Runnable() {
			...
			public void run() {
		    	...
		    	if (!intent.hasExtra(DownloadUtils.MESSENGER_KEY)) {
					Log.e(TAG, "Missing messenger key");
					return;
				}
		    	
		    	Messenger msger = (Messenger)intent.getParcelableExtra(DownloadUtils.MESSENGER_KEY);
		    	if (msger == null) {
		    		Log.e(TAG, "Messenger key does not contain messenger");
		    		return;
		    	}

		    	Uri uri = intent.getData();
		    	if (uri == null) {
		    		Log.e(TAG, "Missing url in intent data");
		    		return;
		    	}
		    	
		    	Log.d(TAG, "Calling downloadAndRespond with " + uri.toString());
		    	DownloadUtils.downloadAndRespond(getApplicationContext(), uri, msger);			}
		};
